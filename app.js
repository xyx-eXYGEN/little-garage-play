  ...
  return [
    {
      ...
      on: false,
      art: sbRefArt(foil),
      bridgeZh: foil.bridgeZh || book.parentCarZh || book.parentStayZh,
    },
  ];
}
  ...
  const answers = `<div class="answer-row belong-row">
    ...
    ${
      got
        ...
        : round.showJoinNudge
          ? `<p class="parent-note">${escapeHtml(round.bridgeZh || book.parentCarZh || book.parentStayZh)}</p>`
          : `<p class="parent-note">跑题了就接住，再问回${escapeHtml(book.topicZh)}。他说车：${escapeHtml(book.parentCarZh || book.parentStayZh)}</p>`
    }`;
  return `${topbar("理解")}
